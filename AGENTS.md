...
3. Run `:checkhealth lazy` and `:Mason` to install LSP servers/tools as needed.
4. If using Mason, open `:Mason` UI and install servers (lua_ls, pyright, tsserver, omnisharp, etc.) or use configured languages.
5. Run formatting chain as needed; Conform is configured for format-on-save (see `lua/noahbalboa66/conform.lua`).
...
- LSP servers are configured in `lua/noahbalboa66/lsp.lua` (or dedicated modules). Mason is used for server installation.
- Formatters are handled via Conform and language-specific formatters are chained where appropriate (e.g., Python: isort -> black).
- Common LSP commands: `:LspInfo`, `:lua vim.lsp.buf.format()` (or configured keymap for format-on-save).
...
- If LSP servers fail to start, open `:Mason` and reinstall the server, then restart Neovim.
...
- Install LSP tools: `:Mason` → install server
- View LSP: `:LspInfo`
